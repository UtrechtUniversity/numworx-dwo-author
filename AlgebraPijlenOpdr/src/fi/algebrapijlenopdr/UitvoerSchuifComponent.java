...
public class UitvoerSchuifComponent extends AlgebraSchuifComponent implements ActionListener, FocusListener
{
	...
	public void setState(Hashtable h)
	{
		...
		/*
		 * if (h.containsKey("imageString")) imageString = (String)
		 * h.get("imageString"); if (imageString.equals("goedkrul"))
		 * pijlUit[0].im = asv.apip.goedkrul; if
		 * (imageString.equals("foutkruis")) pijlUit[0].im = asv.apip.foutkruis;
		 */
	...
	public void paint(Graphics g)
	{
		...
		if (!links)
		{
			...
			g.fillRect(12, labelCorr + 2, getSize().width - 15 - scrollCorr,
				getSize().height - labelCorr - tabelCorr - 5);
			g.setColor(Color.black);
			g.drawRect(12, labelCorr + 2, getSize().width - 15 - scrollCorr,
				getSize().height - labelCorr - tabelCorr - 5);
			...
			else if (expressie != null && expressie.geefVarNaam() != null && !(expressie instanceof Functie)
				&& !kettingZichtbaar)
			{
		...
		else // links
		{
			...
			if (vakKleur != null)
				...
				g.setColor(achtergrondkleur);
			g.fillRect(2, labelCorr + 2, getSize().width - 15 - scrollCorr,
				getSize().height - labelCorr - tabelCorr - 5);
			g.setColor(Color.black);
			g.drawRect(2, labelCorr + 2, getSize().width - 15 - scrollCorr,
				getSize().height - labelCorr - tabelCorr - 5);
	...
	public void zetMaat()
	{
		...
		if (expressie != null && expressie.geefVarNaam() != null && !(expressie instanceof Functie)
			&& !kettingZichtbaar)
		{
	...
	public void zetVeranderd(int max)
	{
		...
		if (expressie != null && expressie.geefVarNaam() != null && getParent() instanceof AlgebraSchuifVeld)
		{
			...
			&& getParent() instanceof AlgebraSchuifVeld)
		{
	...
	/*
	 * public void zetTabel(int beginwaarde, int selectnummer, String varN,
	 * double schaalFactorX) { this.beginwaarde = beginwaarde; this.selectnummer
	 * = selectnummer; if(tabelZichtbaar) { tabel.zetTabel(beginwaarde,
	 * selectnummer, varN, schaalFactorX);
	 * if(grafiekComponent!=null)grafiekComponent.zetTabel(beginwaarde,
	 * selectnummer, varN, schaalFactorX); } zetMaat();
	 * 
	 * }
	 */
	...
	public void setZoomState(String varnaam, ZoomState zoomState)
	{
				...
				&& verborgenExpressie.geefVarNaam().equals(varnaam))
		{
			...
			// if(tabelZichtbaar)
			{
				...
				// if(grafiekComponent!=null)grafiekComponent.zetTabel(beginwaarde,
				// selectnummer, varnaam, schaalFactorX);
			}
			zetMaat();
		}
	...
	{
		tabelAan = b;
		// if(b && !isStapel && (expressie==null
		// ||expressie.geefWaarde()==null))
		if (!isStapel)
			toonTabel(b);
		// else
		// toonTabel(false);
	...
	/*
	 * public String geefBronDefaultVarnaam() { String result = ""; if (pijlIn1
	 * != null) { result = ((UitvoerSchuifComponent)
	 * pijlIn1.zender).geefBronDefaultVarnaam(); } else result = defaultVarnaam;
	 * return result; }
	 */
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == zoomInKnop)
		{
			...
			factorRijNummerX--;
			// ((AlgebraSchuifVeld)getParent()).zetTabellen(beginwaarde,selectnummer,
			// "x", schaalFactorX);
			String varnaam = null;
			if (expressie != null)
				varnaam = expressie.geefVarNaam();
	...
	public void mouseClicked(MouseEvent e)
	{
		...
		{
			if (new Rectangle(tf.getLocation().x, tf.getLocation().y, tf.getSize().width, tf.getSize().height)
	...
	/*
	 * public void mouseDragged(MouseEvent e) { if (!isBeginExpressie)
