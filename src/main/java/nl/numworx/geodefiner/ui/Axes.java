...
public class Axes extends JPanel implements ChangeListener {
	...
	CellItem initItem(Destroyable d, String text) {
		CELL O = d.adapt(CELL.class);
		CELL O1 = new CELL(text, d);
	...
	public void init() {
		Box h = content;
		h.removeAll();
		o = initItem(viewer.getModel().getO(), "$fO = point(0,0)@");
		h.add(o);
		u = initItem(viewer.getModel().getU(), "$fe = point(0,1)@");
		h.add(u);
