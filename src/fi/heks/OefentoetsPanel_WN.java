...
public class OefentoetsPanel_WN extends ScPanel implements ActionListener {
	...
	public OefentoetsPanel_WN(int x, int y, int b, int h, Applet applet, int srt) {
		...
		// kijkNaKnop = new
		// ScLWButton(40,400,140,25,Heks.rb.getString("kijkNaKnopLabel"));
		// kijkNaKnop.addActionListener(this);
		// add(kijkNaKnop);
		...
		if (soort == Som.MIN)
			titelLabel = new ScLabel(20, 20, 200, 40, "Oefentoets 2");
		else if (soort == Som.PLUSMIN)
			titelLabel = new ScLabel(20, 20, 200, 40, "Oefentoets 3");
		else if (soort == Som.MAAL)
			titelLabel = new ScLabel(20, 20, 200, 40, "Oefentoets 5");
		else
			titelLabel = new ScLabel(20, 20, 300, 46, "Typ je antwoord in");
		// add(titelLabel);

		hulpLabel = new ScLabel(550, 260, 170, 20, Heks.rb.getString("aantalHulpLabel"));
		if (Heks.rb.getLocale().toString().equals("en"))
			hulpLabel = new ScLabel(530, 260, 160, 20, Heks.rb.getString("aantalHulpLabel"));
		// add(hulpLabel);

		kerenHulp = new GetalComponent(720, 260, 20, 20);
		if (Heks.rb.getLocale().toString().equals("en"))
			kerenHulp = new GetalComponent(610, 260, 20, 20);
		...
		uitleg = new ScTekstContainer(250, 70, 270, 20, 9, Heks.rb.getString("OefenToetsPanelUitleg"));
	...
	/*
	 * public void start() { int vorigeTerm1 = 0; for(int i=0 ; i<aantalSommen ;
	 * i++) { sommen[i].vernieuw(); sommen[i].geefTerm1();
	 * while(sommen[i].geefTerm1()==vorigeTerm1) {sommen[i].vernieuw(); }
	 * vorigeTerm1 = sommen[i].geefTerm1(); } oefenTafereelPanel.zetGebruikt(0);
	 * scoreLabel.setVisible(false); oefenTafereelPanel.zetActief(false);
	 * kerenHulp.zetWaarde(0); }
	 */
