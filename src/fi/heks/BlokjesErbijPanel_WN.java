...
public class BlokjesErbijPanel_WN extends ScPanel implements ActionListener {
	AppletUtil au;
	...
	ScLWButton helemaalOpnieuwKnop;
	ImageButton volgendeKnop, opnieuwKnop;
	...
	public BlokjesErbijPanel_WN(int x, int y, int b, int h, Applet applet) {
		super(x, y, b, h);
		...
		volgendeKnop = new ImageButton(volgendeknop);
		volgendeKnop.setBounds(5, 300, 96, 25);
		volgendeKnop.addActionListener(this);
		volgendeKnop.setVisible(false);
		add(volgendeKnop);

		opnieuwKnop = new ImageButton(opnieuwknop);
		opnieuwKnop.setBounds(5, 300, 96, 25);
		...
		goedFoutLabel = new ScLabel(150, 345, 100, 30, "");
		goedFoutLabel.setFont(new Font("SansSerif", Font.BOLD, 12));
		if (Heks.rb.getLocale().toString().equals("en"))
			goedFoutLabel = new ScLabel(90, 430, 140, 40, "");
		add(goedFoutLabel);

		titelLabel = new ScLabel(20, 20, 200, 40, Heks.rb.getString("blokjesErinTitelLabel"));
		// add(titelLabel);

		tabelContainer = new ScContainer(5, 30, 750, 300);
	...
	public void zetSom(Som som) {
		...
		if (term2.geefWaarde() > 0)
			aantalBlLabel.setText(Heks.rb.getString("aantalWBlokjesLabel"));
		else
			aantalBlLabel.setText(Heks.rb.getString("aantalKBlokjesLabel"));
		uitkomst.zetBekend(false);
		vraagteken.setVisible(true);
		uitkomst.repaint();
	}
