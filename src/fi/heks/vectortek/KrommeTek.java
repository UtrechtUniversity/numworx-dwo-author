...
{	int aantalBPunten;
	...
	{	boolean bool = false;
		...
		if(Math.abs(a*x-b*y-c)<d  && x>xmin && y>ymin && x<xmax && y<ymax) bool = true;
		return bool;
	}
	
	public boolean contains(int x, int y)
	{	for(int i=0 ; i<aantalTPunten-1 ; i++)
		{	if(opLijn(x,y,tekenpunten[i].x, tekenpunten[i].y, tekenpunten[i+1].x, tekenpunten[i+1].y))return true;
		}
		return false;
	}
	...
	{	
		...
		{	while(!klaar)
			...
			{	if(i==-1)
				...
				double xv0 = x1-x0; double yv0 = y1-y0;
				double xv1 = x2-x1; double yv1 = y2-y1;
				double xv2 = x3-x2; double yv2 = y3-y2;
				
				double uitprodukt1 = xv0*yv1-xv1*yv0;
				double orientatie1;
				if(uitprodukt1==0)orientatie1 = 0;
				else orientatie1 = uitprodukt1/Math.abs(uitprodukt1);
				double improdukt1 = xv0*xv1 + yv0*yv1;
				double norm0 = Math.sqrt((xv0)*(xv0) + (yv0)*(yv0));
				double norm1 = Math.sqrt((xv1)*(xv1) + (yv1)*(yv1));			
				double hoekA =0;
				if(norm0*norm1>0.000010 && Math.abs(improdukt1/(norm0*norm1))<1 )hoekA = orientatie1*Math.acos(improdukt1/(norm0*norm1));
				else if(norm0*norm1>0.000010 && improdukt1/(norm0*norm1)>1)hoekA = 0;
				else if(norm0*norm1>0.000010 && improdukt1/(norm0*norm1)<-1)hoekA = Math.PI;
				double uitprodukt2 = xv1*yv2-xv2*yv1;
				double orientatie2;
				if(uitprodukt2==0)orientatie2 = 0;
				else orientatie2 = uitprodukt2/Math.abs(uitprodukt2);
				double improdukt2 = xv1*xv2 + yv1*yv2;
				double norm2 = Math.sqrt((xv2)*(xv2) + (yv2)*(yv2));
				double hoekB =0;
				if(norm1*norm2>0.000010 && Math.abs(improdukt2/(norm1*norm2))<=1)hoekB = orientatie2*Math.acos(improdukt2/(norm1*norm2));
				else if(norm1*norm2>0.000010 && improdukt2/(norm1*norm2)>1)hoekB = 0;
				else if(norm1*norm2>0.000010 && improdukt2/(norm1*norm2)<-1)hoekB = Math.PI;
				...
				aantalPW++;
				if(Math.sqrt((x1-xn)*(x1-xn) + (y1-yn)*(y1-yn))>2 
				   && Math.sqrt((x2-xn)*(x2-xn) + (y2-yn)*(y2-yn))>2 
