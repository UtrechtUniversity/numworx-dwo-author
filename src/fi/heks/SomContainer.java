...
public class SomContainer extends ScContainer implements ActionListener {
	...
	public void zetSom(Som som, int vaknr) {
		...
		if (vaknr == 0) {
			...
			if (!term1.isInstelbaar())
				term1.zetInstelbaar(true);
			...
			uitkomst.zetBekend(false);
			if (!uitkomst.isInstelbaar())
				uitkomst.zetInstelbaar(true);
			if (!rechtsInvullen)
				uitkomst.setForeground(new Color(150, 0, 0));
	...
	public boolean evalueer() {
		nagekeken = true;
		if ((vaknr == 2 && uitkomst.geefWaarde() == huidigeSom.geefUitkomst()) // &&
																				// term1.isBekend()
				|| (vaknr == 1 && term2.geefWaarde() == huidigeSom.geefTerm2()) // &&
																				// term1.isBekend()
				|| (vaknr == 0 && term1.geefWaarde() == huidigeSom.geefTerm1())) // &&
																					// term1.isBekend()
		{ // goedkrul.setVisible(true);
			goedFoutContainer.removeAll();
			// if(Heks.rb.getLocale().toString().equals("nl")) goedFoutTeken =
			// new Tekening(0,0,(int)relh,(int)relh,au,"goedkrul.gif");
			// else
			...
			return true;
		} else { // foutkruis.setVisible(true);
			goedFoutContainer.removeAll();
