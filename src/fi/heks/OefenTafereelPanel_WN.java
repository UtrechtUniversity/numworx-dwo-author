...
public class OefenTafereelPanel_WN extends ScPanel implements  MouseListener, MouseMotionListener, ActionListener
{	
	...
	public OefenTafereelPanel_WN(int x, int y, int b, int h, Applet applet)
	{	super(x,y,b,h);
		...
		pot = new Tekening(50,40,220,180,au,"potnieuw.gif");
		achtergrond.add(pot);
		...
		blokjePlus = new Tekening(300,30,40,40,au,"blokjePlus.gif");
		achtergrond.add(blokjePlus);
						
		blokjeMin = new Tekening(300,80,40,40,au,"blokjeMin.gif");
		achtergrond.add(blokjeMin);
		
		za = new ZinkAnimatie(75,60,135,100,applet);
		za.zetBellenAan(false);
		sleeppanel.add(za,0);
		
		blokjeSleep = new Tekening(-100,-100,40,40,au,"blokjePlus.gif");
		sleeppanel.add(blokjeSleep,0);
		
		blokjeSleepMin = new Tekening(-100,-100,40,40,au,"blokjeMin.gif");
		sleeppanel.add(blokjeSleepMin,0);
		
		potinhoud = new Tekening(50,60,215,155,au,"inhoudnieuw.gif");
	...
	{	int dx = e.getX() - laatstex;
		...
		{	//plons.play();
			...
			if(!erinMogelijk)blokjeSleep.setLocation(-100,-100);
			else blokjeSleep.setLocation((int)(blokjePlus.getLocation().x),(int)(blokjePlus.getLocation().y));
			raakSleep = false;
			pasEruit = false;
		}
		...
		{	//plons.play();
			...
			if(!erinMogelijk)blokjeSleepMin.setLocation(-100,-100);
			else blokjeSleepMin.setLocation((int)(blokjeMin.getLocation().x),(int)(blokjeMin.getLocation().y));
			raakSleepMin = false;
			pasEruit = false;
		}
