...
public class OefenTafereelPanelEmmer_WN extends ScPanel implements MouseListener, MouseMotionListener, ActionListener {
	...
	public OefenTafereelPanelEmmer_WN(int x, int y, int b, int h, Applet applet) {
		...
		emmerbinnen = new Emmer(120, 320, 140, 155, applet);
		emmerbinnen.addActionListener(this);
		sleeppanel.add(emmerbinnen, 0);

		emmerbuiten = new Emmer(400, 25, 140, 155, applet);
		emmerbuiten.addActionListener(this);
		sleeppanel.add(emmerbuiten);

		emmerSleep = new Emmer(-150, -150, 140, 155, applet);
		sleeppanel.add(emmerSleep, 0);

		za = new ZinkAnimatieEmmer(110, 290, 210, 200, applet);
	...
	public void mousePressed(MouseEvent e) {
		 ...
		 * blokjeSleep.setLocation(e.getX(
		 * )-blokjeSleep.getSize().width/2,e.getY(
		 * )-blokjeSleep.getSize().height/2); blokjeSleep.repaint(); plusEruit =
		 * true; } else if(eruitMogelijk) {
		 * blokjeSleepMin.setLocation(e.getX()-blokjeSleep
		 * .getSize().width/2,e.getY()-blokjeSleep.getSize().height/2);
		 * blokjeSleepMin.repaint(); minEruit = true; }
		 * 
		 * return; } }
		 */
