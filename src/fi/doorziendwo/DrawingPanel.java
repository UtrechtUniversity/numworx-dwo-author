...
// is dit ook OK voor filled = false?
        ...
        switch (code)
        {   case (DoorzienFrame.CUBE):
                model = new Box(1, 1, 1, objectColor);
            break;
            case (DoorzienFrame.BLOCK):
                model = new Box(3, 2, 1, objectColor);
            break;            
            case (DoorzienFrame.TETRAHEDRON):
                model = new Tetrahedron(objectColor);
            break;            
            
            case (DoorzienFrame.OCTAHEDRON):
                model = new Octahedron(objectColor);
            break;            

            case (DoorzienFrame.PIRAMID3):
                model = new Piramid3(objectColor);
            break;            
            case (DoorzienFrame.PIRAMID4):
            {   // four sided, height = base edge
                model = new Piramid(1, objectColor);
            }    
            break;            
            case (DoorzienFrame.PIRAMID5):
                model = new Piramid5(objectColor);
            break;            
            case (DoorzienFrame.PIRAMID6):
                model = new Piramid6(objectColor);
            break;            
            case (DoorzienFrame.PIRAMID7):
                model = new Piramid7(objectColor);
            break;            
            case (DoorzienFrame.PIRAMID8):
                model = new Piramid8(objectColor);
            break;            
            
            case (DoorzienFrame.PRISM3):
                model = new Prism3(objectColor);
            break;            
            case (DoorzienFrame.PRISM4):
                model = new Prism4(objectColor);
            break;            
            case (DoorzienFrame.PRISM5):
                model = new Prism5(objectColor);
            break;            
            case (DoorzienFrame.PRISM6):
                model = new Prism6(objectColor);
            break;            
            
            case (DoorzienFrame.PIRHOUSE):
                model = new PirHouse(objectColor);
            break;            
            case (DoorzienFrame.EDGEHOUSE):
                model = new EdgeHouse(objectColor);
            break;            
            
            case (DoorzienFrame.DODECAHEDRON):
                model = new Dodecahedron(objectColor);
            break;            
            case (DoorzienFrame.ICOSAHEDRON):
                model = new Icosahedron(objectColor);
            break;            
            case (DoorzienFrame.CYLINDER):
                model = new Cylinder(2, 5, objectColor, true, true);
            break;            
            
            case (DoorzienFrame.CONE1):
            {   // foldout 1/4 circle
                // R-bottom = 1/4 * r-foldout
                // r = 4 * R
                // height = sqrt(r^2 - R^2) = 
                // sqrt(15) at R = 1
                model = new Cone(1, Math.sqrt(15), objectColor, true);
            }    
            break;     
            case (DoorzienFrame.CONE2):
            {   // foldout 1/2 circle
                // R-bottom = 1/2 * r-foldout
                // r = 2 * R
                // height = sqrt(r^2 - R^2) = 
                // sqrt(3) at R = 1
                model = new Cone(1, Math.sqrt(3), objectColor, true);
            }    
            break;     
            case (DoorzienFrame.CONE3):
            {   // foldout 2/3 circle
                // R-bottom = 2/3 * r-foldout
                // r = (3/2) * R
                // height = sqrt(r^2 - R^2) = 
                // sqrt(5/4) at R = 1
                model = new Cone(1, Math.sqrt(5d / 4), objectColor, true);
            }    
            break;     
            case (DoorzienFrame.CONE4):
            {   // half top angle 60 degrees
                // height = R / sqrt(3) = 
                // 1 / sqrt(3) at R = 1
                model = new Cone(1, 1 / Math.sqrt(3), objectColor, true);
            }    
            break;     
...
//System.out.println("numPlanes = " + numPlanes);
            ...
            llFactor = 0;
            if (currentObjectGroup instanceof ObjectWithLine)
            {    llFactor = ((ObjectWithLine) currentObjectGroup).getLlFactor();
//System.out.println("OWL ll = " + llFactor);                            
            }
            else if (currentObjectGroup instanceof ObjectWithPlane)
            {
                llFactor = ((ObjectWithPlane) currentObjectGroup).getLlFactor();
            }    
            
//System.out.println("ll = " + llFactor);                
            originalObject.setTickMarks(TICKNUM);
            currentObjectGroup = rebuild(originalObject, construction, null);
            originalObject = currentObjectGroup.leftMostLeaf();                         
    ...
    {   Object3D start = sObject.deepCopy();
        ...
        {   Object ob = recipe.elementAt(i);
            ...
            {   Plane3D pl = (Plane3D) ob;
                int colorIndex = planeOutlineColorIndex;
                if (colors != null)
            ...
            {   Line3D li = (Line3D) ob;
                int colorIndex = lineColorIndex;
                if (colors != null)
                    colorIndex = ((Integer) colors.elementAt(i)).intValue();
                double factor = llFactor;
            ...
            {   Vector3D vertex = (Vector3D) ob;
                int colorIndex = pointColorIndex;
                if (colors != null)
                    colorIndex = ((Integer) colors.elementAt(i)).intValue();
...
//System.out.println("facets = " + foldOutObject.numFacets);                
            ...
            else
            {
                ...
                helpPoint = true;
                panel3D.helpPointColor = planeOutlineColor;
            }
...
// hier moet je dus nog meer resetten voor FI?            
            if (!b)
            {   owner.topToolBar.drawLineButton.setOn(true);
                mouseMode = INERT;
                panel3D.hideHelpLine();                        
                panel3D.hideHelpPoint();                                        
                helpPoint = false;
        ...
        {   
                ...
                )
            {
                ...
                tempObjectGroup2 = 
                    new ObjectWithPoint(tempObjectGroup2, movedPoint, lineColorIndex);
                // remains unchanged                       
            ...
            {   
                ...
                tempObjectGroup2 = 
                    new ObjectWithPoint(tempObjectGroup2, movedEdgeWithPoint, lineColorIndex);
                // remains unchanged                       
...
//setStart();            
                tempObjectGroup = 
                    new ObjectWithPoint(tempObjectGroup, clickedPoint, lineColorIndex);
                    
//showTime("creating OWP-v");                    
                // remains unchanged                       
...
//setStart();                
                tempObjectGroup = 
                    new ObjectWithPoint(tempObjectGroup, clickedEdgeWithPoint, lineColorIndex);
//showTime("creating OWP-e");                    
                // remains unchanged                       
                // originalObject = tempObjectGroup.leftMostLeaf();
                fillPlanes(planesFilled);                
                panel3D.previewModel = null;                
...
// thicken second point via ObjectWithLine?                
                tempObjectGroup2 = 
                    new ObjectWithLine(tempObjectGroup2, 
                        point1, movedPoint, lineColorIndex, 0);
                // remains unchanged                       
                // original2Object = tempObjectGroup2.leftMostLeaf();
                fillPlanes(planesFilled);                                
                panel3D.hideHelpLine();                        
...
// thicken second point via ObjectWithLine?                                
                tempObjectGroup2 = 
                    new ObjectWithLine(tempObjectGroup2, 
                        point1, movedEdgeWithPoint[2], lineColorIndex, 0);
                // remains unchanged                       
                // originalObject2 = tempObjectGroup2.leftMostLeaf();
                fillPlanes(planesFilled);                                
                panel3D.hideHelpLine();        
...
//setStart();                
                currentObjectGroup = new ObjectWithLine(currentObjectGroup,
                    point1, point2, lineColorIndex, llFactor);
...
//System.out.println("co-size = " + construction.length);                        
            ...
            {   // get the line via the mouse, check with construction
                owner.helpBar.setText(owner.tt("selectDeleteLineText"));                            
                owner.topToolBar.deleteLineButton.setPressed(true);
                lineChoosen = null;
                // wait for mouse action
                helpPoint = true;
                panel3D.helpPointColor = lineColor;
...
// uitzondering voor rotate translate        
    ...
    {    
        ...
        if (stepNum == 0)
        {
            ...
            if (!b)
            {   owner.topToolBar.drawPlaneButton.setOn(true);
                mouseMode = INERT;
                panel3D.hideHelpLine();
                panel3D.hideHelpPoint();
                helpPoint = false;
...
//System.out.println("MP and MEWP null");                                
            ...
            {   // replace with point added
                tempObjectGroup = 
                    new ObjectWithPoint(tempObjectGroup, clickedPoint, planeOutlineColorIndex);
                // remains unchanged                       
                // originalObject = tempObjectGroup.leftMostLeaf();
                fillPlanes(planesFilled);                
                panel3D.previewModel = null;                                
            ...
            {   // replace with point added
                tempObjectGroup = 
                    new ObjectWithPoint(tempObjectGroup, clickedEdgeWithPoint, planeOutlineColorIndex);
                // remains unchanged                       
                // originalObject = tempObjectGroup.leftMostLeaf();
                fillPlanes(planesFilled);                
                panel3D.previewModel = null;                                
...
//System.out.println("OWP contains");                                                        
                ...
                currentObjectGroup = new ObjectWithPlane(currentObjectGroup,
                    point1, point2, point3, planeOutlineColorIndex, true);
                // remains unchanged                       
                // originalObject = currentObjectGroup.leftMostLeaf();
                panel3D.previewModel = null;                
                panel3D.hideHelpLine();
...
//System.out.println("co-size = " + construction.size());                        
            ...
            {   Plane3D firstPlane = null;
                ...
                else
                {
                    ...
                    owner.helpBar.setText(owner.tt("selectParPlaneText"));                                        
...
//System.out.println("co-size = " + construction.length);                        
            if (numPlanes > 1)
            {   // get the plane via the mouse
                owner.helpBar.setText(owner.tt("selectDeletePlaneText"));                                        
...
//System.out.println("cut choosen, mouseMode = " + mouseMode);                            
    ...
    {   // als meer vlakken kies een vlak
        ...
        if (stepNum == 0)
        {
            ...
            {
                // help message
                owner.helpBar.setText(owner.tt("selectCutPlaneText"));                
...
{   planeChoosen = null; 
    ...
    return;
            ...
            owner.helpBar.setText(owner.tt("selectCutFigureText"));
            owner.topToolBar.disableLineButtons();            
            owner.topToolBar.disablePlaneButtons();
...
/*            
panel3D.testString = "total = " + UF.format(totalVolume, 3) +
                     " left = " + UF.format(leftVolume, 3) +
                     " right = " + UF.format(rightVolume, 3); 
*/                     
        // neem originalObject
        // snij dit door
...
//System.out.println("not rotated");                        
    ...
    public boolean vertexAllowed(FacetWithVertex fwv)
    {
        ...
        else if (DoorzienFrame.version == DoorzienFrame.EPN)
        {
//setStart();
...
//                        startF = panel3D.clickedFacet(e.getX(), e.getY());                                                    
                    ...
                    {   Facet3D temp = panel3D.clickedFacet(e.getX(), e.getY());
                        if (temp != null)
                        {
...
*/            
        ...
        {   xMoved = e.getX();
            ...
            if (helpPoint)
            {   panel3D.helpPoint = true;
                panel3D.updateHelpPoint(new Point(xMoved, yMoved));
                panel3D.repaint();
            }
            if (mouseMode == DRAWLINE) 
            {   
                // movedPoint and movedEdgeWithPoint stay null
                if (!previewOn)
                {
                    drawLine(pointsSelected + 1, true);    
                    return;
                ...
                // this vertex so use the top object in the tree
                FacetWithVertex fwv = 
                    panel3D.facetWithVertexClicked(xMoved, yMoved);
                ...
                {   
                    ...
                    {
                        // process this point
                        movedPoint = fwv.vertex;
                        drawLine(pointsSelected + 1, true);
                    }
                    else // moved into nothing
                        drawLine(pointsSelected + 1, true);
                }    
                else // no vertex moved into, check for an edge
                {   // ObjectWithPoint finds only visible facets containing
                    // this edge so use the top object in the tree                        
                    FacetWithEdgePoint fwep = 
                        panel3D.facetWithEdgePointClicked(xMoved, yMoved);
                    // restrictions here
                    // 1) only points on the edges of origObject
                    // beter:
                    // 2) points on edges replacing edges of origObject
                    ...
                    {   
                        ...
                        {
                            // process this edge with new point
                            movedEdgeWithPoint = fwep.edgeWithPoint;
                            drawLine(pointsSelected + 1, true);
                        }
                        else // moved into nothing
                            drawLine(pointsSelected + 1, true);
...
*/                
        ...
        {   
            ...
            if (panel3D.helpPoint || helpPoint)
            {   panel3D.helpPoint = true; 
                panel3D.updateHelpPoint(new Point(e.getX(), e.getY()));
                panel3D.repaint();
            }    
...
//System.out.println("mouse exited");            
    ...
    {   boolean dragging = false; 
        ...
        {   if (dragging)
            {
                if ((e.getX() <= 0) || (e.getY() <= 0) ||
                    (e.getX() >= cutPanel.getSize().width) ||
                    (e.getY() >= cutPanel.getSize().height) 
                    )
                {    
                    dragging = false;
                    return;    
                ...
                // choose correct direction
                double centerX = ((double) cutPanel.getSize().width) / 2;
                double centerY = ((double) cutPanel.getSize().height) / 2;                    
                double xTheta = 0;
                double yTheta = 0;

                if (e.getX() < centerX)
                    yTheta = (cutPanel.oldY - e.getY()) * 180.0d /
                             cutPanel.getSize().height;
                else                    
                    yTheta = (e.getY() - cutPanel.oldY) * 180.0d /
                             cutPanel.getSize().height;
                if (e.getY() < centerY)             
                    xTheta = (e.getX() - cutPanel.oldX) * 180.0d /
                             cutPanel.getSize().width;
                else                 
...
class Slider extends Component //implements Observer
{   // sizes, hard coded
...
{   // attributes
    ...
    public void paint(Graphics g)
    {   // background
        g.setColor(DrawingPanel.flatButtonColor);
