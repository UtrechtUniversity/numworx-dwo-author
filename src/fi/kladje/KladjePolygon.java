...
class KladjePolygon
{
	...
	public KladjePolygon()
	{
		
	}
	public KladjePolygon(int[] puntenX, int[]puntenY, int aantalPunten)
	{
	...
	public KladjePolygon (KladjePolygon p)
	{	
	...
	public void draw(Graphics g, Color lijnkleur, Color vulkleur)
  	{
		if (vulkleur != null)
		{	
			g.setColor(vulkleur);
			g.fillPolygon(puntenX, puntenY, aantalPunten);
		}
		if (lijnkleur != null)
		{	
			g.setColor(lijnkleur);
			g.drawPolygon(puntenX, puntenY, aantalPunten);
		}
	...
	public void transformBy(double[] matrix2d, double cx, double cy)
	{
		for (int pCnt = 0; pCnt < aantalPunten; pCnt++)
		{
			double doubleXpCnt = matrix2d[0] * (doubleX[pCnt] - cx) + matrix2d[1] * (doubleY[pCnt] - cy);
			double doubleYpCnt = matrix2d[2] * (doubleX[pCnt] - cx) + matrix2d[3] * (doubleY[pCnt] - cy);
