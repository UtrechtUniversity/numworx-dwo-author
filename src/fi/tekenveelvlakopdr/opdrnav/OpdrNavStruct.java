package fi.tekenveelvlakopdr.opdrnav;
...
public class OpdrNavStruct extends JPanel implements ActionListener
{
	...
	{	int aantalActiviteiten = ((Integer)h.get("aantalActiviteiten")).intValue();
	    ...
	    {	this.aantalOpdrachten[i] = aantalOpdrachten[i];
		    ...
		    {	opdrContainers[i][j].setState(opdrContStates[i][j]);
		    	...
		    	opdrContainers[i][j].correct = orGoedFout[i][j];
	...
	{	int totaalScore = 0; 
	    ...
	    {	String scoreString = scores[i].getText();
	    	...
	    	totaalScore += score;
	    	totaalMax += 10*aantalOpdrachten[i];
	    }
	    double doubleScore = 100*totaalScore/totaalMax;
	...
	{	opdrContainerActief = opdrContainers[actNr][opdrNr];
		opdrContainerActief.setVisible(true);
	...
	public OpdrContainer geefOpdrContainer(int actNr, int opdrNr)
	{	return opdrContainers[actNr][opdrNr];
	}
	...
	{	if(e.getSource() == nakijkKnop)
		...
		{	int score = opdrContainerActief.getScore();
			...
			scores[activiteitNr].setText("Score: "+totaal);
			if(correct) produceAction("correct");
