...
public class MozArch extends TekenApplet implements ActionListener
{
	...
	{	panel = new Panel();		panel.setBounds(1,471,600,28);
		...
		boolean opsturenMogelijk = false;
		String opstuurCode = getParameter("servlet");
		if(opstuurCode!=null && opstuurCode.equals("true"))
		{	opsturenMogelijk = true;
		}
		
		boolean dataBekijkenMogelijk = false;
		String bekijkCode = getParameter("servletplus");
		if(bekijkCode!=null && bekijkCode.equals("true"))
		{	dataBekijkenMogelijk = true;
		}
		
		String opdrCode = getParameter("opdrachtnr");
		if(opdrCode!=null)
		{	opdrachtNr = Integer.parseInt(opdrCode);
		}
		...
		if(opsturenMogelijk)panel.add(inzendenKnop);
		...
		if(dataBekijkenMogelijk)panel.add(bestandenKnop);
	...
	public void zetOpsturenMogelijk(boolean b)
	{	if(b)
		{	panel.add(inzendenKnop);
		}
		else
		{	panel.remove(inzendenKnop);
		}
	}
	
	public void zetDataBekijkenMogelijk(boolean b)
	{	if(b)
		{	panel.add(bestandenKnop);
		}
		else
		{	panel.remove(bestandenKnop);
		}
	}
	...
	{	penUit();stap(-390,250);penAan();
		...
		if(opdrachtNr==4)
		{	stap(100,-50);
			stap(0,-380);stap(390,0);stap(0,380);stap(-390,0);
			stap(-100,50);
			vulUit();
			penUit();
			stap(1,-1);
			vulAan(220,220,160);
			stap(99,0);stap(0,-49);stap(-99,0);stap(0,49);
		}
		else
		{	stap(10,-10);
	...
	{	trekx =0 ; treky = 0;
		...
		if(!actiefVlakdeel.nieuw && (actiefVlakdeel.draaipunt.x<-400 || actiefVlakdeel.draaipunt.x>200
		   || actiefVlakdeel.draaipunt.y<-280 || actiefVlakdeel.draaipunt.y>270))
	...
	{	if(e.getSource()==wisKnop)
		...
		{	if(bestandenFrame!=null)bestandenFrame.dispose();
			...
			bestandenFrame.zetOpdrachtNr(opdrachtNr);
			bestandenFrame.show();
