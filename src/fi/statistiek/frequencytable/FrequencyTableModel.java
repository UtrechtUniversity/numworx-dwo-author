 ...
 *
 */
public class FrequencyTableModel extends Observable implements TableModelListener, SelectionListener, StatBinsModel {
	 ...
	 * @param bins The new bin boundaries
	 */
	 ...
	 * @param noBins the new number of bins
	 */
	public void setNoBins(int noBins) {		
		...
		if(this.tableModel.getRowCount()>0 && this.columnIndexValid()) {
			...
			this.binBoundaries = Statistiek.appropriateBoundaries(min, max, noBins);
		}
	 ...
	 * @param columnIndex The index of the column that will be shown
	 */
	public void setColumnIndex(int columnIndex) {
//		System.out.println("setColumnIndex op frequencytablemodel");
		if(!(this.columnIndex == columnIndex)) {
			this.columnIndex = columnIndex;
			if(this.columnIndexValid() && this.tableModel.getColumnTypes().get(this.columnIndex).getType().isNumber()) {
				this.binBoundaries = Statistiek.appropriateBoundaries(this.tableModel.getColumnMin(this.columnIndex), this.tableModel.getColumnMax(this.columnIndex), this.binBoundaries.size()-1);
			}
			this.setChanged();
			this.notifyObservers();
		}
	...
	/**
	 * @return The index of the column that is represented by this StatistiekView
	 */
	...
	public boolean columnIndexValid() {
		return this.columnIndex>=0 && this.columnIndex<this.tableModel.getColumnCount();
	}
	
	public int binOfNumber(double d) {
		int bin = -1;
		while(bin < this.binBoundaries.size()-1 && d>this.binBoundaries.get(bin+1)) {
			bin++;
		}
	...
	/**
	 * Find the frequency of every bin, and the amount of selected objects in this bin
	 * Only use for columns of type integer or double
	 * @return array of frequencies, with index 2*i the frequency of bin i, and 2*i + 1 the amount of selected items in this bin.
	 */
	public int[] numberClassFrequency() {
		return this.tableModel.numberClassFrequency(this.binBoundaries, this.columnIndex, null)[0];
	}
	
	/**
	 * Find the frequency of every class
	 * Only use for columns of type enum of string
	 * @return array of FrequencyTuples (which contains class label and frequency)
	 */
