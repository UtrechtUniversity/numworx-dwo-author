...
//	private JButton dialogButton;
	 ...
	 */
	public FrequencyTableView(FrequencyTableModel model, FrequencyTableController controller) {
		...
		this.mainPanel = new JPanel() {
			public void paintComponent(Graphics g) {
				...
				double columnWidth = this.getWidth()/(double)FrequencyTableView.this.mainPanelColumns;
				int rowAreaHeight = this.getHeight() - (FrequencyTableView.this.mainPanelRows+1)*FrequencyTableView.MAIN_GRID_HGAP;
				
				double rowHeight = (rowAreaHeight/(double)(FrequencyTableView.this.mainPanelRows+2)) + MAIN_GRID_HGAP;
				...
				if(FrequencyTableView.this.rowColors != null) {
					for(int i = 0; i < FrequencyTableView.this.rowColors.length; i++) {
						...
						g.fillRect(0, (int)Math.round((i+1)*rowHeight), this.getWidth(), (int)Math.round((i+2)*rowHeight)-(int)Math.round((i+1)*rowHeight));
					}
				}
				...
				int y = (int)Math.round((FrequencyTableView.this.mainPanelRows+1) * roundRowHeight);
		...
		this.columnIndexLabel = new JLabel(Statistiek.rb.getString("variableLabel"), SwingConstants.RIGHT);
		...
		this.showPercBox = new JCheckBox(Statistiek.rb.getString("showpercentageCheckbox"), true);
		...
		this.showFreqBox = new JCheckBox(Statistiek.rb.getString("showfrequencyCheckbox"), true);
		...
		this.noBinsLabel = new JLabel(Statistiek.rb.getString("numberofbinsLabel"), SwingConstants.RIGHT);
		...
		this.chooseBinsButton = new JButton(Statistiek.rb.getString("BoundariesButton"));
		...
		this.showCumulativeBox = new JCheckBox(Statistiek.rb.getString("showcumulativefrequencyCheckbox"), true);
		...
		dialogButton = new DialogButton(Statistiek.rb.getString("settingsButton"),this.userOptionsPanel);
	...
	public void update(Observable arg0, Object arg1) {
		...
		this.noBinsField.setText(Integer.toString(this.model.getBinBoundaries().size()-1));
		...
		if(this.model.columnIndexValid()) {
			ColumnType cType = this.model.getTableModel().getColumnTypes().get(this.model.getColumnIndex());
			...
			GridLayout gl = new GridLayout(this.mainPanelRows+2, this.mainPanelColumns);
			gl.setHgap(FrequencyTableView.MAIN_GRID_HGAP);
			gl.setVgap(FrequencyTableView.MAIN_GRID_VGAP);
			this.mainPanel.setLayout(gl);
			this.mainPanel.setBackground(Color.WHITE);
			//make header row
			this.mainPanel.add(new JLabel(this.model.getTableModel().getColumnName(this.model.getColumnIndex())));
			if(this.model.isShowFreq()) {
				this.mainPanel.add(new JLabel("Freq.", SwingConstants.TRAILING));
			}
			if(this.model.isShowPercentage()) {
				this.mainPanel.add(new JLabel("Freq.%", SwingConstants.TRAILING));
			}
			if(this.model.isShowCumulative()) {
				this.mainPanel.add(new JLabel("Cumul.", SwingConstants.TRAILING));
			}
			if(this.model.isShowPercentage()) {
				this.mainPanel.add(new JLabel("Cumul.%", SwingConstants.TRAILING));
			}
			...
			for(int bin = 0; bin < this.mainPanelRows; bin++) {
				if(type.isNumber()) {
					...
					JLabel label = new JLabel(this.model.getBinBoundaries().get(bin).toString() + " - " + this.model.getBinBoundaries().get(bin+1).toString());
				...
				if(this.model.isShowFreq()) {
					this.mainPanel.add(new JLabel(Integer.toString(freq), SwingConstants.TRAILING));
				}
				if(this.model.isShowPercentage()) {
					double d = freq*100/(double)sum;
					d = Math.round(d*100)/(double)100;
					this.mainPanel.add(new JLabel(Double.toString(d) + "%", SwingConstants.TRAILING));
				}
				if(this.model.isShowCumulative()) {
					cumulative += freq;
					this.mainPanel.add(new JLabel(Integer.toString(cumulative), SwingConstants.TRAILING));
				}
				if(this.model.isShowPercentage()) {
			...
			//make total row
			this.mainPanel.add(new JLabel(Statistiek.rb.getString("totalLabel")));
			if(this.model.isShowFreq()) {
				this.mainPanel.add(new JLabel(Integer.toString(sum), SwingConstants.TRAILING));
			}
			if(this.model.isShowPercentage()) {
			...
			if(type.isNumber()) {
				...
				for(int i = 0; i < frequencies.length; i+=2) {
					double d = (frequencies[i] == 0 ? 0.0 : (double)frequencies[i+1]/(double)frequencies[i]);
					System.out.println(frequencies[i+1] + " " + frequencies[i] + " " + d);
					this.rowColors[i/2] = ColorPreviewer.mixColors(Color.WHITE, SELECTED_COLOR, d);
				}
			}
			else {
				...
				for(int i = 0; i < frequencyTuple.length; i++) {
					...
					double d = (ft.frequency == 0 ? 0 : (double)ft.selectionFrequency/(double)ft.frequency);
					System.out.println(d);
					this.rowColors[i] = ColorPreviewer.mixColors(Color.WHITE, SELECTED_COLOR, d);
				}
			}
		}
	...
	private class RowClickListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent arg0) {
			int rowAreaHeight = FrequencyTableView.this.mainPanel.getHeight() - (FrequencyTableView.this.mainPanelRows+1)*FrequencyTableView.MAIN_GRID_HGAP;
		...
		private void rowClicked(int row) {
			...
			ColumnType cType = FrequencyTableView.this.model.getTableModel().getColumnTypes().get(FrequencyTableView.this.model.getColumnIndex());
			AllowedTypes type = cType.getType();
			if(type.isNumber()) {
				ArrayList<Boolean> selectionList = new ArrayList<Boolean>(FrequencyTableView.this.model.getTableModel().getRowCount());
				for(int i = 0; i < FrequencyTableView.this.model.getTableModel().getRowCount(); i++) {
					Object o = FrequencyTableView.this.model.getTableModel().getValueAt(i, FrequencyTableView.this.model.getColumnIndex());
					
					selectionList.add(o.equals(ColumnType.WILDCARD) || FrequencyTableView.this.model.binOfNumber(Double.parseDouble((String)o))==row);
				}
				
				FrequencyTableView.this.model.getTableModel().setSelectionList(selectionList);
			}
			else {
				String clicked;
				if(type.equals(AllowedTypes.ENUM)) {
					clicked = cType.getEnumOptions()[row];
					int wildcardIndex = Arrays.asList(cType.getEnumOptions()).indexOf(ColumnType.WILDCARD);
					if(wildcardIndex <= 0 && wildcardIndex < row) {
						clicked = cType.getEnumOptions()[row+1];
					}
				}
				else {
					ArrayList<String> options = FrequencyTableView.this.model.getTableModel().stringColumnOptions(FrequencyTableView.this.model.getColumnIndex());
				...
				for(int i = 0; i < FrequencyTableView.this.model.getTableModel().getRowCount(); i++) {
					Object o = FrequencyTableView.this.model.getTableModel().getValueAt(i, FrequencyTableView.this.model.getColumnIndex());
					selectionList.add(o.equals(ColumnType.WILDCARD) || ((String)o).equals(clicked));
				}
				FrequencyTableView.this.model.getTableModel().setSelectionList(selectionList);
