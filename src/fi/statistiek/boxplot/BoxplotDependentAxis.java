...
public class BoxplotDependentAxis extends JPanel {
	...
	/**
	 * Constructor
	 * @param dataMinValue the lowest value in the data
	 * @param dataMaxValue the highest value in the data
	 * @param verticalBoxplots true if using vertical boxplots, false otherwise
	 * @param boxplotView the BoxplotView that uses this
	 * @param columnName the name of the column thats represented by this
	 */
	public BoxplotDependentAxis(double dataMinValue, double dataMaxValue, boolean verticalBoxplots, BoxplotView boxplotView, String columnName) {
	...
	private int valueToScreenLocation(double d) {
		...
		double y = (a*SingleBoxplotView.HEIGHT_FILL_FRACTION)* (this.verticalBoxplots ? super.getHeight() : super.getWidth());
		y += (1.0-SingleBoxplotView.HEIGHT_FILL_FRACTION)*0.5*(this.verticalBoxplots ? super.getHeight() : super.getWidth());
	...
	private void determineScale() {
		...
		double min = this.dataMinValue - (1-SingleBoxplotView.HEIGHT_FILL_FRACTION)*0.5*(this.dataMaxValue-this.dataMinValue);
		this.max = this.dataMaxValue + (1-SingleBoxplotView.HEIGHT_FILL_FRACTION)*0.5*(this.dataMaxValue-this.dataMinValue);
	...
	public void paintComponent(Graphics g) {
		...
		if(this.verticalBoxplots) {
			...
			while(p < max) {
				...
				g.drawString(Double.toString(p), super.getWidth()-7-fm.stringWidth(Double.toString(p)), y+(int)(0.5*fm.getHeight()));
			...
			while(p < max) {
				...
				g.drawLine(x, 0, x, 5);
				g.drawString(Double.toString(p), x - (int)(fm.stringWidth(Double.toString(p))/2), 7 + fm.getHeight());
