...
public class BoxplotController implements StatistiekView, ActionListener {
	 ...
	 */
	public BoxplotController(StatTableModel tableModel, String viewName, int startVar) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(actionCommand.equals("varBox")) {
			...
			if(selectedString != null) {
				this.model.setColumnIndex(this.model.getTableModel().getColumnIndexByName(selectedString));
				this.view.repaint();
			}
		}
		else if(actionCommand.equals("horizontalboxplotsRadio") || actionCommand.equals("verticalboxplotsRadio")) {
			this.model.setVerticalBoxplots(this.view.isVerticalBoxesButtonSelected());
		}
			...
			if(c instanceof Dialog) {
				dialog = new SplitOptionsDialog((Dialog)c, this.model.getSplitOptions(), this.model.getTableModel());
			}
			else if(c instanceof Frame) {
				dialog = new SplitOptionsDialog((Frame)c, this.model.getSplitOptions(), this.model.getTableModel());
			}
			...
			if(this.view.getSplitVarBoxSelectedIndex()-1 != this.model.getSplitOptions().getColumnSplitIndex()) {
				this.model.setColumnSplitIndex(this.view.getSplitVarBoxSelectedIndex()-1);
				this.model.setSplitOptions(this.model.getSplitOptions());
				if(this.view.getSplitVarBoxSelectedIndex() > 0) {
					this.setSplitType(this.model.getTableModel().getColumnTypes().get(this.model.getSplitOptions().getColumnSplitIndex()).getType());
				}
	...
	private void updateSplitBoundaries() {
		...
		for(int i = 0; i <= this.view.getSplitBinsBoxSelectedInt(); i++) {
			boundaries.add(new Double(view.getSplitminBoundary() + i * view.getSplitBinWidth()));
		}
		this.model.setSplitBinBoundaries(boundaries);
		this.view.setModel(this.model);
	}
	...
	public void setState(Object state) {
		...
		if(h.containsKey("splitBoundaries")) {
			this.model.setSplitBinBoundaries((ArrayList<Double>)h.get("splitBoundaries"));
		}
