...
public class SplitOptions {
	...
	public String getSplitClassLabel(int splitClass, StatTableModel model) {
		ColumnType splitCType = model.getColumnTypes().get(this.columnSplitIndex);
		if(splitCType.getType().isNumber()) {
			return this.binBoundaries.get(splitClass) + " - " + this.binBoundaries.get(splitClass+1);
		}
		else if(splitCType.getType().equals(AllowedTypes.ENUM)) {
			return splitCType.getEnumOptions()[splitClass];
		}
		else {
			return model.getStringOptions(this.columnSplitIndex).get(splitClass);
		}
	}
