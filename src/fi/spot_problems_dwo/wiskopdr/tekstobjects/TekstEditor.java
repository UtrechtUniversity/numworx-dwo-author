package fi.spot_problems_dwo.wiskopdr.tekstobjects;
...
public class TekstEditor extends Panel implements ActionListener, MouseListener, AdjustmentListener
{	
	private Image im;
	private Graphics gIm;
	
	private FormuleButton formuleKnop,wortelKnop, machtKnop, breukKnop, kwadraatKnop, ndewortelKnop, haakjesKnop;
	...
	{	if(scrollbar)scrollPane.setBounds(b-31,23,21,h-17);
		else scrollPane.setBounds(b,23,20,h-5);
		contentPane.setBounds(10,25,b-20,h-37);
		contentPaneNep.setBounds(0,2,0,h-37);
		p1.setBounds(b-31,h-12,21,12);
		p2.setBounds(b-31,h-2,21,2);
		p3.setBounds(b-31,h-11,21,1);
		p4.setBounds(b-31,25,2,h-37);
		tekstVak.setBounds(0,0,b-40,h-30);
	...
	{	int max = 0; //scrollPane.getSize().height-20;
		...
		contentPane.setSize(contentPane.getSize().width, max);
		contentPaneNep.setSize(contentPaneNep.getSize().width, max);
		scrollPane.doLayout();
		//contentPaneNep.setSize(contentPaneNep.getSize().width, max);
		//contentPane.repaint();
	}
