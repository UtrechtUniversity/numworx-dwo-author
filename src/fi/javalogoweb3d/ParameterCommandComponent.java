...
public abstract class ParameterCommandComponent extends SimpleCommandComponent implements ParameterEditorListener
{
	...
	private void setParameterPositions(int editIndex)
	{
		...
		for ( int i=0; i<noParameters; i++ )
		{
			...
			{							// add width of TextField
				currentX = currentX+paramEditor.getWidth();
			} else
	...
	/**
	 * Get a string value of the command in this componenent, with the formulas in the parameters
	 * written out, not calculated. 
	 * 
...
//		String s = tab + getCommandName() + strOpen +  getFullParameterText() + strClose + "\n";
		String s = tab + getCommandNameTranslated() + strOpen +  getFullParameterText() + strClose + "\n";
		return s;
	}
	...
	{
		String s = getCommandNameTranslated() + strOpen;
		for ( int i=0; i<noParameters; i++ )
		{
