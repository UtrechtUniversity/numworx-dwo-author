...
{	
	...
	{
		// Fix PBgv, 2015-08-16: 2x setLocation had (incorrectly) been commented out!!
		// adjusted to new flexible size by always referring to programmaPanel's dimension
		if ( isWide )
		{
			setSize(pcsw, getHeight());
			setLocation(Math.min(narrowX, schuifveld.getPPDimension().width-pcsw), getY());
			isWide = false;
		} else
		{
			setSize(pclw, getHeight());
			narrowX = getX();
			setLocation(Math.min(getX(), schuifveld.getPPDimension().width-pclw), getY());
			isWide = true;
		}
	}
	...
	void changeHeight()
	{
		...
		setLocation(getX(), Math.min(getY(), Math.max(0,schuifveld.getPPDimension().height-newh)));
	}
	...
	public boolean execute(TraceBeheerder trb, TekenApplet3D ub, VarSet varSet)
	{
		...
		trb.setCommandInfo(JavaLogoWeb3d.rb.getString("Klaar"), varSet);
		return false;
	}
