package fi.statsim.text;

public class Text_en extends java.util.ListResourceBundle
{
        public Object [] [] getContents() { return contents; }
        static final Object [] [] contents =
        {
            {	"settings" , "Settings" },
            {	"oneCoin" , "One coin" },
            {	"twoCoins" , "Two coins" },
            {   "numberOfRounds", "Number of rounds" },
            {   "chanceOfTails", "Chance of tails" },
            {   "start", "Start" },
            {   "next", "Next" },
            {   "stop", "Stop" },
            {   "results", "Results" },
            {   "mean", "Average" },
            {   "minimum", "Minimum" },
            {   "maximum", "Maximum" },
            {   "numberOfHeads", "Number of heads" },
            {   "percentageHeads", "Percentage heads" },
            {   "noHeads", "0 heads" },
            {   "oneHeads", "1 heads" },
            {   "twoHeads", "2 heads" },
            {   "exp", "exp." },
            {   "numberOfTails", "Number of tails" },
            {   "frequency", "Frequency" },
            {   "heads", "Heads" },
            {   "tails", "Tails" },
            {   "percentageHeads", "Percentage heads" },
            {   "eyes", "Eyes" },
            {   "sumEyes", " Sum eyes" },
            {   "numberOfDices", "Number of dices" },
            {   "one", "One" },
            {   "two", "Two" },
            {   "three", "Three" },
            {   "showSum", "Show sum" },
            {   "outcome", "Outcome" },
            {   "numberOfDraws", "Number of draws" },
            {   "chance", "Chance" },
            {   "times", "times" },
            {   "so", "so" },
            {   "notso", "not so" },
            {   "step", "Step" },
            {   "graph", "Graph" },
            {   "grid", "Grid" },
            {   "execute", "Execute" },
            {   "erase", "Erase results" },
            {   "execute1", "Execute" },
            {   "coins", "Coins" },
            {   "table", "Table" },
            {   "dice", "Dice" },
            {   "binominalDraw", "Binominal draw" },
            {   "grid", "Grid" },
            {   "CBA_text.sample", "CSV Sample" },
            {   "CBA_text.sampleProportion", "CSV Sample Proportion" },
            {   "CBA_text.sampleCollection", "CSV Sample collection" },
            {   "sample", "Sample" },
            {   "sampleSize", "Sample size" },
            {   "getSample", "Get sample"},
            {   "getSample100Times" , "Get sample 100 times" },
            {   "value", "Value" },
            {   "leftTable", "Left table" },
            {   "rightTable", "Right table" },
            {   "steekproefInstellingenZichtbaar", "Settings visible"},
            {   "scheveVerdeling", "Example skewed distribution"},
            {   "populationProportion", "Population proportion" },
            {   "extraLabel", "Extra" },
            {	"extraAantalDobbelstenenLabel" , "Number of dices ="},
            {	"extraAantalMuntenLabel" , "Number of coins ="},
            {	"extraAantalKeerGooienLabel" , "Number of throws ="},
            {	"extraGooienLabel" , "Throw"},
            {	"extraOpnieuwLabel" , "Again"},
            {	"extraAantalKerenGegooidLabel" , "Times throwed"},
            {	"extraTotaalGegooidLabel" , "Total times throwed"},
            {	"extraTotaalGetalLabel" , "Total number"},
            {	"extraVerschilGetalLabel" , "Difference"},
            {	"extraDobbelsteenLabel" , "dice"},
            {	"extraHead" , "Head"},
            {	"extraTail" , "Tail"},
            {	"extraEn" , "and"},
        //    {   "sampleSize", "Sample size" }
        //	{	"button1Label" , "text1_en" },
		//	{ 	"button2Label" , "text2_en" },
		//	....
		
		};
}




