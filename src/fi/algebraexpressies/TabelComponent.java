...
public class TabelComponent extends JPanel //Component 
...
{	
	...
	{	int breedte = getSize().width;
		...
		pijlPlus.addPoint(3 + breedteInv + 3 + breedteUitv / 2 - 5, 12);
		pijlPlus.addPoint(3 + breedteInv + 3 + breedteUitv / 2 + 5, 12);
		pijlPlus.addPoint(3 + breedteInv + 3 + breedteUitv / 2, 4);
		g.fillPolygon(pijlPlus);
		g.drawPolygon(pijlPlus);
		
		pijlMin = new Polygon();
		pijlMin.addPoint(3 + breedteInv + 3 +breedteUitv/2-5,hoogte-13);
		pijlMin.addPoint(3 + breedteInv + 3 +breedteUitv/2+5,hoogte-13);
		pijlMin.addPoint(3 + breedteInv + 3 +breedteUitv/2,hoogte-5);
...
*/
	...
	{	setCursor(new Cursor(Cursor.HAND_CURSOR));
		...
		if (beginx == beginxOud)
		{	for (int i = 0; i < 8; i++)
			{	if ((new Rectangle(5, 15 + i * 15, getSize().width - 10, 15)).contains(e.getX(),e.getY()))
				{	if (selectnummer == i)
						selectnummer = 999;
					else 
						selectnummer = i;
