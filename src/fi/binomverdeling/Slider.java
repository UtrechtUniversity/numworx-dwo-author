...
/**
 * Overgenomen uit Normale Verdeling
 * enabled toegevoegd door Manu
 */
public class Slider extends JComponent implements MouseListener, MouseMotionListener {
	...
	public void tekenSlider(Graphics g) {
		 ...
		 * schuifKnop.addPoint(5+stand+3,15); schuifKnop.addPoint(5+stand,20);
		 * schuifKnop.addPoint(5+stand-3,15); schuifKnop.addPoint(5+stand-3,5);
		 * g.fillPolygon(schuifKnop); g.drawPolygon(schuifKnop);
		 */
	...
	public void mouseDragged(MouseEvent e) {
		if (this.editable) {
			if (!raak && enabled && (new Rectangle(stand, 0, 10, 20).contains(e.getX(), e.getY()))) {
				raak = true;
				muisStartX = e.getX();
				if (raak && actionListener != null) {
					actionListener.actionPerformed(new ActionEvent(this, 0,"start"));
	...
	public void mouseExited(MouseEvent e) { // raak = false;
	}

	public void mouseEntered(MouseEvent e) {
	}

	public void mouseMoved(MouseEvent e) { // if (new Rectangle(stand, 0, 10,
		// 20).contains(e.getX(), e.getY()))
