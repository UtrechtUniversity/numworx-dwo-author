package fi.algebrapijlenopdr.expressies;
...
{	
	...
	{	String[] varNamen = geefVarNamen(e);
		...
		{	Vector u = geefFactoren((Expressie)v.elementAt(i),new Vector());
			...
			{	Expressie exp = (Expressie)u.elementAt(k);
				if(exp instanceof BasisExpressie  && ((BasisExpressie)exp).basisString.equals(varNaam))
				{	exponenten[i]++;
		...
		{	Expressie ee = (Expressie)v.elementAt(i);
			...
			{	String varnaam;
				...
				{	Expressie eee = (Expressie)v.elementAt(j);
					...
					else if(eee instanceof Deling && eee.kind1.geefWaarde()==1 && eee.kind2 instanceof BasisExpressie && ((BasisExpressie)eee.kind2).basisString.equals(varnaam))
					{	aantalF--;
						v.setElementAt(new BasisExpressie(1),j);
					}
				}
	...
	{	if(e.kind1!=null)e.kind1 = herleidMild(e.kind1);
		...
		{	if(e.kind1.geefWaarde()==1) return e.kind2;
			...
			else if(e.kind1.geefWaarde()<0) return new Aftrekking(new BasisExpressie(0),new Vermenigvuldiging(new BasisExpressie(-e.kind1.geefWaarde()),e.kind2));
			else if(e.kind2.geefWaarde()<0) return new Aftrekking(new BasisExpressie(0),new Vermenigvuldiging(new BasisExpressie(-e.kind2.geefWaarde()),e.kind1));
		}
	...
	{	String[] varNamen = geefVarNamen(e);
		...
		{	Expressie exp = (Expressie)u.elementAt(k);
			for(int i=0 ; i<varNamen.length ; i++)
			{	if(exp instanceof BasisExpressie  && ((BasisExpressie)exp).basisString.equals(varNamen[i]))
				{	exponenten[i]++;
					u.setElementAt(new BasisExpressie(1),k);
				}
				else if(exp instanceof Deling && exp.kind1.geefWaarde()==1  && exp.kind2 instanceof BasisExpressie  && ((BasisExpressie)exp.kind2).basisString.equals(varNamen[i]))
				{	exponenten[i]--;
					u.setElementAt(new BasisExpressie(1),k);
				} 
			}				
