...
public class AftrekVak extends RegelVak
{	
	private String operator;
	
	public AftrekVak(FormuleVak  fv)
	{	formuleVak = fv;
		operator = " -  ";
	...
	{	//zetMaat();
		...
		int x = kind1.getSize().width;
		int y = ashoogte;//+fm.getAscent()/2+ fm.getDescent()/4;
		g.drawLine(x+fm.getAscent()/4,y+fm.getAscent()/5,x+fm.getAscent()/4+fm.getAscent()/2,y+fm.getAscent()/5);
		//g.drawString(operator,kind1.getSize().width,ashoogte+fm.getAscent()/2+ fm.getDescent()/4);
		super.paint(g);
		//super.paint(g);
	}
