...
public class VergelijkingMeerv
{
	...
	/*
	 * public boolean bevatFouteOplossing(VergelijkingMeerv antw) { boolean
	 * isOplossing = true; for(int j=0 ; j<vergelijkingen.length ; j++) {
	 * if(vergelijkingen[j].isEindOplossing()) { isOplossing =
	 * antw.isOplossing(vergelijkingen[j].geefEindOplossing());
	 * if(!isOplossing)return true; } } return false; }
	 */
	...
	{
		if (isVectorVergelijking())
		{
			// TODO vars bepalen
			//return isStelselOplossing(subst, vars);
		}
	...
	public boolean isStelselOplossing(Expressie[][] subst, String[] vars)
	{
		for (int i = 0; i < subst.length; i++)
		{
			if (!isStelselOplossing(subst[i], vars))
				return false;
		}
		return true;
	}
	...
	public String toStringStrikt()
	{
		...
		for (int i = 1; i < vergelijkingen.length; i++)
		{
			s = s + "  " + WiskOpdr.rb.getString("ofLabel") + "  " + vergelijkingen[i].toStringStrikt();
		}
	...
	/**
	 * public boolean isEindOplossing() { for(int i=0 ; i<vergelijkingen.length
	 * ; i++) { if(!vergelijkingen[i].isEindOplossing())return false; } return
	 * true; }
	 */
	...
	public boolean isEindOplossingSignificant(Expressie[][] subst, String var, String[] vergTekens)
	{
		for (int i = 0; i < subst.length; i++)
		{
			if (!isEindOplossingSignificant(subst[i], var, vergTekens[i]))
				return false;
		}
		return true;
	}
	...
	public boolean isStelselEindOplossing(String var, String[] vars)
	{
		for (int i = 0; i < vergelijkingen.length; i++)
		{
			if (!vergelijkingen[i].isStelselEindOplossing(var, vars))
				return false;
		}
		return true;
	}
	...
	/**
	 * public double[] geefEindOplossing() { double[] oplossingen = new
	 * double[vergelijkingen.length]; if(isEindOplossing()) { for(int i=0 ;
	 * i<vergelijkingen.length ; i++) { oplossingen[i] =
	 * vergelijkingen[i].geefEindOplossing(); } } return oplossingen; }
	 */
