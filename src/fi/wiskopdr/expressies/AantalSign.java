...
{	
	...
	{	
		kind1 = e1;
		if(e1 instanceof Aftrekking && e1.kind1.geefWaarde()==0) kind1 = e1.kind2;
		isVeelterm = false;
		isProdukt = false;
		isBasis = false;
	}
	...
	{	double aantal = Double.NaN;
		//if(Double.isNaN(kind1.geefWaarde()))return aantal;
		//else 
			
		if(kind1 instanceof BasisExpressie && !(kind1 instanceof PI) && !(kind1 instanceof E)){
			String s = ((BasisExpressie)kind1).geefBasisString();//toString();
			...
			//System.out.println("aantalSignString: "+s);
			return s.length();
