...
{	double waarde = Double.NaN;
	...
	public Sigma( Expressie e1, Expressie e2, Expressie e3, Expressie e4)
	{	kind1 = e1;
		kind2 = e2;
		kind3 = e3;
		kind4 = e4;
	...
	{	if(var.equals(kind2.geefVarNaam()))	return this;			
		return new Sigma(kind1.substitueer(subst,var), kind2.substitueer(subst,var), kind3.substitueer(subst,var), kind4.substitueer(subst,var));
	}
	...
	{
		return new Sigma(kind1.vervangDiffs(subst, var), kind2.vervangDiffs(subst, var), kind3.vervangDiffs(subst, var), kind4.vervangDiffs(subst, var));
	}
	...
	{	String s1 = kind1.geefVarNaam();
		...
		else if(s1!=null && s2!=null && !s1.equals(s2) && !s1.equals(s2))return "";
	...
	{	return "$S" + kind1.toString() + "$n" + kind2.toString() + "$k" + kind3.toString() + "$l" + kind4.toString() + "@@@@";//"$n" + kind3.toString() + 
	}
    ...
    {   if(Double.isInfinite(kind4.geefWaarde()))return "Sum[" + kind1.toStringCAS() + ",{" + kind2.toStringCAS()+ "," + kind3.toStringCAS() + ",Infinity}]";
    	else return "Sum[" + kind1.toStringCAS() + ",{" + kind2.toStringCAS()+ "," + kind3.toStringCAS() + "," + kind4.toStringCAS() + "}]";
    }
