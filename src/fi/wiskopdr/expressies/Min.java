...
public class Min extends Expressie  
{	double waarde = Double.NaN;

	static final double NZERO = 1e-5d;
	
	public Min(Expressie e1, Expressie e2)
	{	kind1 = e1;
		kind2 = e2;
	...
	{	
		...
		double waarde;
		if(Double.isNaN(m) || Double.isNaN(n)) waarde = Double.NaN;
		else waarde = Math.min(m,n);			
		return waarde;
	}
	...
	{	return new Min(kind1.substitueer(subst,var), kind2.substitueer(subst,var));
	}
	...
	{	return new Min(kind1.vervangDifferentialen(var), kind2.vervangDifferentialen(var));
	}
	
	public Expressie vervangDiffs(Expressie subst, String var)
	{
		return new Min(kind1.vervangDiffs(subst, var), kind2.vervangDiffs(subst, var));
	}
    ...
    public Object visit(AbstractConverter converter) {
    	return converter.min( kind1.visit(converter), kind2.visit(converter));
    }
