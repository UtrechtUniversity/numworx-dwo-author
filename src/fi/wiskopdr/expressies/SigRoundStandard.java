...
public class SigRoundStandard extends Expressie  
{	
	
	public SigRoundStandard(Expressie e1, Expressie e2)
	{	kind1 = e1;
		kind2 = e2;
		isVeelterm = false;
		isProdukt = false;
	...
	{	
		...
		int aantal = (int)kind2.geefWaarde();
		return roundToSignificantFigures(d,aantal);
	}
	
	public Complex geefWaardeComplex(double subst)
	{	return new Complex(roundToSignificantFigures(kind1.geefWaarde(subst), (int)kind2.geefWaarde(subst)));
	}
	
	public double geefWaarde(double[] subst, String[] vars)
	{	return roundToSignificantFigures(kind1.geefWaarde(subst,vars), (int)kind2.geefWaarde(subst,vars));
	}
	
	public Expressie substitueer(double subst, String var)
	{	SigRoundStandard exp = new SigRoundStandard(kind1.substitueer(subst,var), kind2.substitueer(subst,var));
	...
	{	//if(isWaarde())return new BasisExpressie(geefWaarde()).toString();
		return "rns" + "$h" + kind1.toString() + "_" + kind2.toString() + "@";
	}
