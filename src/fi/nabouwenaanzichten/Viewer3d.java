...
public class Viewer3d extends JPanel //JComponent
{
	...
	private double yhoek, beginx, beginy;
	...
	{	grZichtbaar = bool;
		...
		{	int n = kr.maxAantal;
			...
			int x = breedte * 80 / 300;
			int y = hoogte * 80 / 300;
			int b = hoogte * 140 / 300;

			if (hoogte < breedte)
			{	x += (breedte - hoogte) * 80 / 300;
				
			}
			else // hoogte > breedte
			{
				x -= (hoogte - breedte) * 80 / 300;
			}
...
//System.out.println("b = " + b);
	...
	void tekenprogramma()
	{	
//System.out.println("a1 = " + mat.getAantalRotaties());		
		
		mat.initialiseer();
		
//System.out.println("a2 = " + mat.getAantalRotaties());

		mat.xdraai(beginx + xhoek);
		mat.ydraai(beginy + yhoek);
		
//System.out.println("a3 = " + mat.getAantalRotaties());		
		tekenKubusRooster();		
	}
...
//System.out.println("sch = " + startschaal);
...
//    	g.drawImage(im, 0, 0, null);
  	...
  	public void tekenOpImage(boolean wis, Graphics gIm)
  	{ 	if (gIm == null)
  			return;
			...
			for (int i = 0; i < l[j].aantalPolygonen; i++)
			{
				...
				{	if (schaduw)
					{	double grijsfactor = 0.5 * 
							((-l[j].vlakken[i].normaal.x - l[j].vlakken[i].normaal.y + 
							  l[j].vlakken[i].normaal.z) / Math.sqrt(3) + 1);
						if (grijsfactor < 0)
							grijsfactor = 0;
						if (grijsfactor > 1)
							grijsfactor = 1;
...
*/
		...
		if (p instanceof NabouwenAanzichtenInteractiePanel)
		{
//System.out.println("naip");
			((NabouwenAanzichtenInteractiePanel) p).repaint();
		}
		else
		{
			repaint();
...
//System.out.println("naip");
	...
	{	removing = false;
        if (!removed && 
        	(klikAan && (mb.geefDrukx() - mb.geefX()) * (mb.geefDrukx() - mb.geefX()) + 
	...
	{	
		...
		{	try
			...
			{}
    		if ((removing && !dood && klikAan && 
    			(mb.geefDrukx() - mb.geefX()) * (mb.geefDrukx() - mb.geefX()) + 
    			(mb.geefDruky() - mb.geefY()) * (mb.geefDruky() - mb.geefY()) < 16) )
    		{	removed = true;
    			muisKkActie(ee, true);
    			eigenaar.zetVeranderd();	
