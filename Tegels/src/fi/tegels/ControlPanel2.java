...
{	
	...
	public ControlPanel2(TegelsPanel gv)
	{	
		eigenaar = gv;
		...
		int width = Math.max(fm.stringWidth(Tegels.rb.getString("legknopLabel")), 
		...
		{	final Color buttonColor = kleuren[i];
			...
			kleurenV[i].setBounds(arrowButtonWidth + offset + 20 * i, 
						          //draaiknop.getLocation().y + draaiknop.getSize().height + offset,
								  eigenaar.controlHoogte - offset / 2 - 20,	
						          20 , 20);
			add(kleurenV[i]);
		...
		gridKeuze = new ActKeuzePanel(items, 
									  ...
									  //draaiknop.getLocation().y + draaiknop.getSize().height + offset / 2,
									  eigenaar.controlHoogte - offset / 2 - height,									  
