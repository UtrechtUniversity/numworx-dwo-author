...
// mag aanklikken!!!!!!
    ...
    public Facet3D(Vector3D[] pts, int[] inds, Color c)
    {   // set reference
//        allPoints = pts;
        // init indices
...
// NOTE ob != null requests exact drawing through edge coverings       
        Polygon p = project(dis, mat.origin);
...
// hier kijken of exacte tekening gewenst             
        ...
        {   // -1 is not thickened
            ...
            {    //boolean hidden = false;
                 ...
                 if ((vertexCodes[cnt] >= 0) && (vertexCodes[cnt] < 10))
                 {
                     ...
                     {   g.setColor(getHiddenOutlineColor(
                             edgeColors[vertexCodes[cnt]]));
                     }        
                     else // not "hidden"
        ...
        {   
            ...
            for (int vCnt = 0; vCnt < p.npoints; vCnt++)
            {
                ...
                {
                    int bx = p.xpoints[vCnt];
                    int by = p.ypoints[vCnt];
                    int ox = (int) mat.origin.x;
                    int oy = (int) mat.origin.y;
                    int shiftX = ox - bx;
                    if (shiftX >= 0)
                        bx -= (3 + 
                               vertexFM.stringWidth(vertexLabels[vCnt]));
                    else
                        bx += 3;
                    int shiftY = oy - by;
                    // y-axis reversed!
...
//System.out.println("steps = " + steps);
    ...
    public static Color getHiddenOutlineColor(Color c)    
    {   Color result = c;
...
// tijdelijk
    ...
    // find shadowed color
    public static Color shadowColor(Facet3D f, boolean reverseNormal)
