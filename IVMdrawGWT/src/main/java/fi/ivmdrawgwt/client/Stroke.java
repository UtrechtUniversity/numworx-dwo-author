...
//			return null;
	...
	public Stroke(ArrayList<DoublePoint> points, boolean fromDoublePoints) {
		ArrayList<DoublePoint> doublePoints = new ArrayList<DoublePoint>();
		for(int i = 0 ; i < points.size() ; i++) {
			doublePoints.add(new DoublePoint(points.get(i).getX(), points.get(i).getY()));
		}
...
//		}
		...
		for(int i = 0 ; i < parsePoints.size() ; i++) {
			parsePointsIntX.add((int)(parsePoints.get(i).x*10));
			parsePointsIntY.add((int)(parsePoints.get(i).y*10));
		}
