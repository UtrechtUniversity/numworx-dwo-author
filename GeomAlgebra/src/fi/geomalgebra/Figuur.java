...
{	int aantalx, aantaly;
	...
	{	if(new Rectangle(positie.x-4, positie.y-4, 8, 8).contains(x,y))return true;
		else return false;
	}
	
	public boolean raakStaartX(int x, int y )
	{	if(new Rectangle(posx.x-4, posx.y-4, 8, 8).contains(x,y))return true;
		else return false;
	}
	
	public boolean raakStaartY(int x, int y )
	{	if(new Rectangle(posy.x-4, posy.y-4, 8, 8).contains(x,y))return true;
		else return false;
	}
	
	public boolean raakRechthoek(int x, int y )
	{	if(new Rectangle(minx, miny, maxx-minx, maxy-miny).contains(x,y))return true;
		if(miny==maxy && new Rectangle(minx, miny-2, maxx-minx, 4).contains(x,y))return true;
		if(minx==maxx && new Rectangle(minx-2, miny, 4, maxy-miny).contains(x,y))return true;
		return false;
	}
	
	public boolean raakSplits(int x, int y )
	{	for(int i=0 ; i<aantalx ; i++)
		{	if(new Rectangle(lsx[i].positie.x-4, lsx[i].positie.y-4, 8, 8).contains(x,y))return true;
		}
		for(int i=0 ; i<aantaly ; i++)
		{	if(new Rectangle(lsy[i].positie.x-4, lsy[i].positie.y-4, 8, 8).contains(x,y))return true;
		}
		return false;
	}
	...
	{	Figuur f = null;
		for(int i=1 ; i<aantalx ; i++)
		{	if(new Rectangle(lsx[i].positie.x-4, lsx[i].positie.y-4, 8, 8).contains(x,y))
		...
		for(int i=0 ; i<aantaly ; i++)
		{	if( new Rectangle(lsy[i].positie.x-4, lsy[i].positie.y-4, 8, 8).contains(x,y))
