...
// wat gesnoeid, alleen verplaatsfocus
...
import fi.grafiek3dtest.tekstobjects.TekstRegel;
...
{	
	...
	{	int b=1;
		...
		{	//FormuleElement fe = (FormuleElement)getComponent(getComponentCount()-1);
			...
			setSize(b+correctieCursief,h1+h2);
			ashoogte = h1;
		}
		else 
		{	setSize(fm.getAscent()/2, fm.getAscent() + fm.getDescent());
	...
	{	if(Grafiek3DTest.mobileVersion)
		{	if(waiting) return;
			waiting = true;
	        if(showPopupThread!=null)
			{	showPopupThread.maakDood();
				showPopupThread=null;
			}
	        showPopupThread = new ShowPopupThread(e);
