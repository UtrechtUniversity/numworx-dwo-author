...
{	
	...
	{	super.setFont(f);
		...
		kind1.setLocation(5*fm.getAscent()/12,fm.getAscent()/12-1);
		kind1.setFont(f);
	}
	...
	{	//zetMaat();
		...
		g.drawLine(c, d, c, hoogte-d);		
		g.drawLine(breedte-1-c, d, breedte-1-c, hoogte-d);
	...
	public void zetMaat()
	{	setSize(5*fm.getAscent()/6 + kind1.getSize().width,  kind1.getSize().height);
		ashoogte = kind1.ashoogte + fm.getAscent()/12;
