...
// elke as een apart Object3D?        
    		...
    		for (int xCnt = 0; xCnt < (numXFineFacets + 1); xCnt++)
    		{
    			...
    			{	if ((xCnt == 0) || (xCnt == numXFineFacets))
    					vLabels[xCnt] = "x = " + df.format(xMin + xCnt * xStepFine);
        			else if ((xCnt % xFinerSteps) == 0)
        				vLabels[xCnt] = df.format(xMin + xCnt * xStepFine);
        			else
        				vLabels[xCnt] = "";
    			}
    		...
    		for (int yCnt = 0; yCnt < (numYFineFacets + 1); yCnt++)
    		{
    			...
    			{	if ((yCnt == 0) || (yCnt == numYFineFacets))
    					vLabels[numXFineFacets + 1 + yCnt] = "y = " + df.format(yMin + yCnt * yStepFine);
        			else if ((yCnt % yFinerSteps) == 0)
        				vLabels[numXFineFacets + 1 + yCnt] = df.format(yMin + yCnt * yStepFine);
    				else
    					vLabels[yCnt] = "";
    			}
    			else if (labelType == Grafiek3DComponent.ENDLABELS)
    		...
    		for (int zCnt = 0; zCnt < (numZFacets + 1); zCnt++)
    		{
    			...
    			{	if ((zCnt == 0) || (zCnt == numZFacets))
    					vLabels[numXFineFacets + 1 + numYFineFacets + 1 + zCnt] = "z = " + df.format(zMin + zCnt * zStep);
        			else
        				vLabels[numXFineFacets + 1 + numYFineFacets + 1 + zCnt] = df.format(zMin + zCnt * zStep);
    			}	
    			else if (labelType == Grafiek3DComponent.ENDLABELS)
    			{	if ((zCnt == 0) || (zCnt == numZFacets))
    					vLabels[numXFineFacets + 1 + numYFineFacets + 1 + zCnt] = "z = " + df.format(zMin + zCnt * zStep);
...
//elke as een apart Object3D?        
			...
			{	vertices[2 * (xFinerSteps + 1) + (yFinerSteps + 1) + yMaxCnt] = 
				...
				{	if (yMaxCnt == yFinerSteps)
						vLabels[2 * (xFinerSteps + 1) + (yFinerSteps + 1) + yMaxCnt] = 
							"F" + "y = " + df.format(yMax - yStep + yMaxCnt * yStepFine);
					else
						vLabels[2 * (xFinerSteps + 1) + (yFinerSteps + 1) + yMaxCnt] = "";
				}
				else if (labelType == Grafiek3DComponent.ENDLABELS)
	        ...
	        for (int zCnt = 0; zCnt < (numZFacets + 1); zCnt++)
	        {
	        	...
	        	if (labelType == Grafiek3DComponent.ALLLABELS)
	        	{	if ((zCnt == 0) || (zCnt == numZFacets))
        				vLabels[numPointVertices + numFloorVertices + zCnt] = "z = " + df.format(zMin + zCnt * zStep);
	        		else
	        			vLabels[numPointVertices + numFloorVertices + zCnt] = df.format(zMin + zCnt * zStep);
	        	}	
	        	else if (labelType == Grafiek3DComponent.ENDLABELS)
	        	{	if ((zCnt == 0) || (zCnt == numZFacets))
	        			vLabels[numPointVertices + numFloorVertices + zCnt] = "z = " + df.format(zMin + zCnt * zStep);
