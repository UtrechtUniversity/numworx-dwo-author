R=~/Public/jars

cp ../target/wiskopdr.jar $R/wiskopdr.jar
cp ../target/wiskopdr.jar.pack.gz $R/wiskopdr.jar.pack.gz
cp ../target/graphtool.jar $R/graphtool.jar
cp ../target/graphtool.jar.pack.gz $R/graphtool.jar.pack.gz
cp ../target/balansfruitapplet.jar $R/balansfruitapplet.jar
cp ../target/balansfruitapplet.jar.pack.gz $R/balansfruitapplet.jar.pack.gz
cp ../target/geodefiner.jar $R/geodefiner.jar
cp ../target/geodefiner.jar.pack.gz $R/geodefiner.jar.pack.gz
cp ../target/statistiek.jar $R/statistiek.jar
cp ../target/statistiek.jar.pack.gz $R/statistiek.jar.pack.gz
cp ../target/geogebra3widget.jar $R/geogebra3widget.jar
cp ../target/geogebra3widget.jar.pack.gz $R/geogebra3widget.jar.pack.gz
cp ../target/geogebra4widget.jar $R/geogebra4widget.jar
cp ../target/geogebra4widget.jar.pack.gz $R/geogebra4widget.jar.pack.gz
