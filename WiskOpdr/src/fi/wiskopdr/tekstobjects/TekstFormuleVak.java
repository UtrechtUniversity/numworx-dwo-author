...
public class TekstFormuleVak extends TekstDeelVak implements ActionListener, MouseListener
{
	...
	{	super(tv);
		...
		Font f = new Font("TimesRoman", tv.getFont().getStyle(), tv.getFont().getSize()*5/4);//+2);
		fm = getFontMetrics(f);
		if(!WiskOpdr.formTimes || WiskOpdr.mac || WiskOpdr.zoefi) f = tv.getFont();
		setFont(f);
		add(formuleVak);
	...
	{	super.setFont(font);
		Font f = new Font("TimesRoman", font.getStyle(), font.getSize()*6/5);//+2);
		if("TimesRoman".equals(font.getName()) || !WiskOpdr.formTimes || WiskOpdr.mac || WiskOpdr.zoefi) f = font;
