...
// last change:
	...
	*/
	...
	private boolean backSpaceNeedsLayout() {
		...
		boolean b4 = geefVolgendeRegel(actieveRegel)!=null  && actieveRegel.getWidth() + geefVolgendeRegel(actieveRegel).geefBreedteStartWoord() - 1 > breedte-2*marge;
		return !b1 && !b2 && !b3 && !b4;
	}
	...
	private boolean deleteNeedsLayout() {
		boolean b2 = tekst.toString().substring(caretPos).startsWith("\n"); // cursor voor newLine
		boolean b3 = actieveRegel.toString().contains("\n"); // newLine in actieve regel
		boolean b4 = geefVolgendeRegel(actieveRegel)!=null  && actieveRegel.getWidth() + geefVolgendeRegel(actieveRegel).geefBreedteStartWoord() - 8 > breedte-2*marge;
		return b2 || !b3 && !b4;
	}
