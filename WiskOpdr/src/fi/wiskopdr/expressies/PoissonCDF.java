...
public class PoissonCDF extends Expressie  
{	double waarde = Double.NaN;

	static final double NZERO = 1e-5d;
	
	public PoissonCDF(Expressie e1, Expressie e2)
	{	kind1 = e1;
		kind2 = e2;
		isVeelterm = false;
		isProdukt = false;
	...
	public double poissonCum(double lambda, int x)
	{	double cum = 0;
		for(int i=1 ; i<x+1 ; i++)
		{	cum = cum + poisson(lambda, i);
		}
		return cum;
	}
	...
	{
		return new PoissonCDF(kind1.vervangDiffs(subst, var), kind2.vervangDiffs(subst, var));
	}
	...
	{	return "poissoncdf"  + "$h" +  kind1.toString() + "_" + kind2.toString() + "@";
	}
    ...
    public Object visit(AbstractConverter converter) {
    	return converter.poissoncdf(kind1.visit(converter), kind2.visit(converter));
    }
