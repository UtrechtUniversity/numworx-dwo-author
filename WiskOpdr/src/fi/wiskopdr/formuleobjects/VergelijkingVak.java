...
public class VergelijkingVak extends RegelVak
{	
	private String vergTeken;
	
	public VergelijkingVak(FormuleVak  fv)
	{	formuleVak = fv;
		vergTeken = " = ";
		...
		setSize(fm.getAscent()/2 + fm.stringWidth(vergTeken),fm.getAscent() + fm.getDescent());
		...
		kind2.setLocation(fm.getAscent()/2 + fm.stringWidth(vergTeken),0);
		add(kind2);
		setOpaque(false);
	}
	...
	{	super.setFont(f);
		...
		int b = fm.stringWidth(vergTeken) + kind1.getSize().width + kind2.getSize().width;
		...
		kind2.setLocation(kind1.getSize().width + fm.stringWidth(vergTeken),ashoogte-kind2.ashoogte);
		kind1.setFont(f);
		kind2.setFont(f);
	...
	{	//zetMaat();
		...
		g.drawString(vergTeken,kind1.getSize().width,ashoogte+fm.getAscent()/2+ fm.getDescent()/4);
		super.paint(g);
		super.paint(g);
	}
