...
public class TekenApplet3D extends JPanel
{
	...
	{	//this.setLayout(null);
		...
		//tb.setBounds(0,0,getSize().width-(heeftRg?150:0),getSize().height);
		//add(tb,0);
		//rg.setBounds(getSize().width-150,0,150,getSize().height);
		//if(heeftRg)add(rg,0);
...
{	
	...
	{	InvoerVariabele iv = new InvoerVariabele(n,mn,mx,val);
		...
		NumberArrow invoercomp = new NumberArrow(iv.geefMin(), iv.geefMax(), iv.geefWaarde(), 1, 0, iv.geefNaam(), "");
		invoercomp.addNumberListener(iv);
		invoercomp.setBackground(Color.white);
		gridbag.setConstraints(invoercomp, c);
		add(invoercomp);
		return iv;
		}
...
class Tekenblad3D extends JPanel
{
	...
	public Matrix3D mat;  
	private TekenApplet3D eigenaar;
	private boolean pen, vul,leeg, schaduw, schalend;
	private int lnummer;
	...
	public void zetSchalend(boolean b)
	{	schalend = b;
	}
...
class AnimatieBeheerder extends JPanel implements ActionListener, Runnable
{
	...
	{	eigenaar = ap;
		animatieAan = false;
		animatieknop = new JButton("animatie");
...
class MuisBeheerder implements MouseListener, MouseMotionListener
{
	...
	private TekenApplet3D eigenaar;
	private TekenApplet3D eigenaar1;
	...
	{	if(eigenaar!=null && eigenaar.animatieStatus())
		{	animatieWasAan = true;
		...
		laatstex = e.getX();
		laatstey = e.getY();
		if(eigenaar!=null)eigenaar.muisDrukActie();
		if(eigenaar1!=null)eigenaar1.muisDrukActie();
	}
	...
	{	int x = e.getX();
		...
		dx = x - laatstex;
		dy = laatstey -y;
		if(eigenaar!=null)eigenaar.muisSleepActie();
		if(eigenaar1!=null)eigenaar1.muisSleepActie();
		laatstex = x;
		laatstey = y;	
	}
	
	public void mouseReleased(MouseEvent e)
	{	if(eigenaar!=null)eigenaar.muisLosActie();
		if(eigenaar1!=null)eigenaar1.muisLosActie();
		if(animatieWasAan)
