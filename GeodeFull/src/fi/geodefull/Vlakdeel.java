...
{	
	...
	public Component eigenaar;
	public boolean nieuw;
	//public boolean tekenbaar;
	
	public Vlakdeel(Component ap,int aantal, double positiex, double positiey, Color kl)
	{	nieuw = true;
		aantalPunten = aantal;
	...
	{	beginnummer = hoekn;
		if(hoekpunten[hoekn].aantalVastgeklikt==0)aantalHoekpuntenVast++;
		hoekpunten[hoekn].maakVast(vlakdeeln, hoeknVlakdeel);
		draaipunt = new Punt(hoekpunten[hoekn].tekenpunt.x-0.5*eigenaar.getSize().width,-hoekpunten[hoekn].tekenpunt.y+0.5*eigenaar.getSize().height );
  	}
	...
	{	hoekpunten[hoekn].maakLos(vlakdeeln, hoeknVlakdeel);
		...
		{	beginnummer = 0;
			draaipunt = new Punt(hoekpunten[0].tekenpunt.x-0.5*eigenaar.getSize().width,-hoekpunten[0].tekenpunt.y+0.5*eigenaar.getSize().height );
		}
	}
...
class HoekpuntMoz
{
	...
	{	for(int i=0 ; i<aantalVastgeklikt ; i++)
		...
		if(aantalVastgeklikt < 1)vast = false;
	}
	
	boolean zitVast(int vlakdeeln,int hoeknVlakdeel)
	{	boolean b=false;
		for(int i=0 ; i<aantalVastgeklikt ; i++)
		{	if(vlakdeelnummers[i]==vlakdeeln && hoeknummersVlakdeel[i]==hoeknVlakdeel)b=true;
		}
		return b;
	}
}