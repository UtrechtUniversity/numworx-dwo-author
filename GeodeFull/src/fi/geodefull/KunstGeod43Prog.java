...
public class KunstGeod43Prog extends TekenApplet3D 
{	
	...
	{	matrot = new Matrix3D();
		...
		{	for(int j=1 ; j<g.aantalDrPerFacet+1 ; j++)
			{	if(j==4 || j==5 || j==6 || j==28 || j==29 || j==30 ||
				   j==34 || j==35 || j==36 || j==31 || j==32 || j==33 ||
					j==22 || j==23 || j==24)g.dr[i][j].vulkleur = "geel";
				else g.dr[i][j].vulkleur = "cyaan";
				g.dr[i][j].lijnkleur = "transparant";
			}
		}
